...
# Form implementation generated from reading ui file 'DialogWithOpenDeposit.ui'
#
...
class Ui_OpenNewAccount(object):
    def setupUi(self, OpenNewAccount, banks):
        ...
        self.DepositSum = QtWidgets.QLineEdit(OpenNewAccount)
        font = QtGui.QFont()
        font.setPointSize(8)
        self.DepositSum.setFont(font)
        self.DepositSum.setAlignment(QtCore.Qt.AlignCenter)
        self.DepositSum.setObjectName("DepositSum")
        self.horizontalLayout_3.addWidget(self.DepositSum)
        self.Result = QtWidgets.QLineEdit(OpenNewAccount)
        font = QtGui.QFont()
        font.setPointSize(8)
        ...
        self.retranslateUi(OpenNewAccount)
        self.BankChoice.setCurrentIndex(0)
        self.DepositChoice.setCurrentIndex(0)
        self.BankChoice.currentTextChanged['QString'].connect(self.DepositChoice.setCurrentText) # type: ignore
        QtCore.QMetaObject.connectSlotsByName(OpenNewAccount)

    def retranslateUi(self, OpenNewAccount):
        _translate = QtCore.QCoreApplication.translate
        OpenNewAccount.setWindowTitle(_translate("OpenNewAccount", "BelBank"))
        self.text.setText(_translate("OpenNewAccount", "Депозит"))
        ...
        self.DepositChoice = QtWidgets.QComboBox(OpenNewAccount)
        self.DepositChoice.setEditable(False)
        self.DepositChoice.setMaxCount(2147483637)
        self.DepositChoice.setSizeAdjustPolicy(QtWidgets.QComboBox.AdjustToContents)
        self.DepositChoice.setMinimumContentsLength(0)
        self.DepositChoice.setObjectName("DepositChoice")
        self.DepositChoice.setCurrentText(_translate("OpenNewAccount", self.banks[0].name))
        self.set_deposits_choice(self.BankChoice.currentText())
        self.DepositSum.setPlaceholderText(_translate("OpenNewAccount", "Сумма депозита"))
    ...
    def set_deposits_choice(self, bank_name):
        _translate = QtCore.QCoreApplication.translate
        for i in range(self.amount_of_deposits):
            self.DepositChoice.removeItem(0)
        self.amount_of_deposits = 0
        for bank in self.banks:
            if bank.name == bank_name:
                for i in range(len(bank.deposits_list)):
                    self.DepositChoice.addItem("")
                    self.amount_of_deposits += 1
                    self.DepositChoice.setItemText(i, _translate("OpenNewAccount", f"{bank.deposits_list[i].days} дней ({bank.deposits_list[i].percent}%)"))
        self.verticalLayout_4.addWidget(self.DepositChoice)
        try:
            self.Result.setText(f"{round(float(self.DepositSum.text()) * (1 + (float(self.DepositChoice.currentText().split()[2][1:-2]) / 100)))}")
        except:
            pass
