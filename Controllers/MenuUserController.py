from Views import MenuUser, DialogWithOpenAccount, DialogWithOpenCredit, DialogWithOpenDeposit, DialogWithTrancsaction
from Models import MenuUserModel


class MenuUserController():
    def __init__(self, user):
        self.view = MenuUser.Ui_MenuUser()
        self.model = MenuUserModel.MenuUserModel(self.view, user)
...
#########################################################################################################
    ...
    def press_confirm_transaction(self):
        try:
            if int(self.view_transactions.my_accounts.currentText().split(" | ")[1]) >= int(self.view_transactions.sum.text()):
